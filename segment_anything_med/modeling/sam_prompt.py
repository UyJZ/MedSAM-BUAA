...
class Prompt_Block(nn.Module):
    ...
    def __init__(self, block, prompt_param):
...
class Sam(nn.Module):
    ...
    def __init__(
        ...
        if prompt_layer is not None:
            ...
            # Default to all layers (Deep Prompt Tuning)
        ...
        for i, block in enumerate(self.image_encoder.blocks):
            ...
            # Shape: (1, grid_size, grid_size, embed_dim) to match (B, H, W, C)
            prompt_param = nn.Parameter(torch.zeros(
                1, grid_size, grid_size, embed_dim))

            # Initialization (Xavier Uniform is a common choice)
            nn.init.xavier_uniform_(prompt_param)

            self.prompt_embeddings.append(prompt_param)
