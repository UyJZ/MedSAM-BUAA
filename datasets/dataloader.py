...
class GlaSDataset(Dataset):
    ...
    def __init__(self, root_dir, split='train', val_ratio=0.2, target_size=512, seed=42):
        """
        Args:
        ...
        # 4. 根据比例切分
        split_idx = int(len(self.images) * (1 - val_ratio))
...
if __name__ == "__main__":
    ...
    # 假设你的数据集文件夹名叫 "Dataset_BUSI_with_GT"
    # 请根据你实际解压出来的文件夹名字修改这里
    DATASET_ROOT = "/Users/shizixuan/Desktop/研一上/医学影像处理/project/Dataset_BUSI"

    print(f"正在读取数据集路径: {DATASET_ROOT}")

    if os.path.exists(DATASET_ROOT):
        # 实例化 Dataset
        train_dataset = BUSIDataset(DATASET_ROOT, split='train', val_ratio=0.1)
        val_dataset = BUSIDataset(DATASET_ROOT, split='val', val_ratio=0.1)

        print(f"总计图片: {len(train_dataset) + len(val_dataset)}")
        ...
        for i, batch in enumerate(loader):
            ...
            print(f"[{i}] 正在处理: {name[0]}")
            print(f"   Image Shape: {image.shape}, Type: {image.dtype}")
            print(f"   Mask Shape: {mask.shape}, Type: {mask.dtype}")

            # --- 可视化 ---
            # 转回 Numpy (C, H, W) -> (H, W, C)
            img_vis = (image[0].permute(1, 2, 0).numpy() * 255).astype(np.uint8)
            img_vis = cv2.cvtColor(img_vis, cv2.COLOR_RGB2BGR)  # BGR for OpenCV

            mask_vis = (mask[0][0].numpy() * 255).astype(np.uint8)

            # 画 BBox
            x1, y1, x2, y2 = bbox[0].int().numpy()
            cv2.rectangle(img_vis, (x1, y1), (x2, y2), (0, 255, 0), 2)  # 绿色框

            # 拼接显示
            # 将 mask 转为 3通道以便拼接
            mask_vis_bgr = cv2.cvtColor(mask_vis, cv2.COLOR_GRAY2BGR)
            combined = np.hstack((img_vis, mask_vis_bgr))

            cv2.imshow(f"BUSI Sample - {name[0]}", combined)
        ...
        print(f"\n错误: 找不到文件夹 '{DATASET_ROOT}'")
        print("请确保你下载并解压了 BUSI 数据集，并将文件夹放在代码同一目录下。")
